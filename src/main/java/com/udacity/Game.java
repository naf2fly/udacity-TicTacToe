...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        //for diagonals
        for(int i=0;i<2;i++) {
            if (xo[i] == grid[0][0] && xo[i] == grid[1][1] && xo[i] == grid[2][2]) {
                won = true;
                result = xo[i] + " won";
            }
            if (xo[i] == grid[2][0] && xo[i] == grid[1][1] && xo[i] == grid[0][2]) {
                won = true;
                result = xo[i] + " won!";
            }
        }

        //x for horizontal and vertical wins
        for (int i=0;i<3;i++){
            if ((xo[0] == grid[0][i] && xo[0] == grid[1][i] && xo[0] == grid[2][i]) || (xo[0] == grid[i][0] && xo[0] == grid[i][1] && xo[0] == grid[i][2]))
            {
                won = true;
                result = "x won!";
                }
        }
        // for horizontal and vertical wins
        for (int i=0;i<3;i++){
            if ((xo[1] == grid[0][i] && xo[1] == grid[1][i] && xo[1] == grid[2][i]) || (xo[1] == grid[i][0] && xo[1] == grid[i][1] && xo[1] == grid[i][2]))
            {
                won = true;
                result = "o won!";
            }
